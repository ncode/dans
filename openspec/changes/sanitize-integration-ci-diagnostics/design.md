...
5. The unchanged macOS development-shell CI contract timed out with no test output, while its local equivalent passed. Add fixed, nonsecret phase markers at the test's major boundaries and unbounded waits. Use the next CI run to locate the stall, then replace the specific indefinite wait with a bounded failure and a focused regression check. Keep raw captured process output private and preserve the existing test assertions.
...
- macOS CI behavior is not reproducible locally → Phase markers identify the actual stalled boundary without publishing process details; no wait is changed until that boundary is observed.
