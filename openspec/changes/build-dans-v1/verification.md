...
| Command | Result |
| --- | --- |
| `rtk make generate-check` | OpenAPI and sqlc generation matched the checked-in output. |
| `rtk go test ./... -count=1` | 462 tests passed across 14 packages. |
| `rtk go test -race ./... -count=1` | 462 tests passed across 14 packages with the race detector. |
| `rtk go vet ./...` | Clean. |
| `rtk go mod verify` | All modules verified. |
| `rtk go tool sqlc vet -f sqlc.yaml` | Clean. |
| `rtk sh -n scripts/integration.sh` | Integration harness syntax valid. |
| `rtk scripts/integration-contract.sh` | Integration and CI contract passed. |
| `rtk scripts/release_workflow_test.sh` | Release-workflow contract passed. |
| `rtk openspec validate build-dans-v1 --strict` | Change valid in strict mode. |
...
| --- | --- | --- |
| 16.14 | `rtk env DANS_QA_LOG_DIR=/private/tmp/dans-qa-freeze2-pg16 scripts/integration.sh postgres:16.14` | `integration: PostgreSQL 16.14 passed` |
| 18.4 | `rtk env DANS_QA_LOG_DIR=/private/tmp/dans-qa-freeze2-pg18 scripts/integration.sh postgres:18.4` | `integration: PostgreSQL 18.4 passed` |

## Release and deployment artifacts

| Command | Result |
| --- | --- |
| `rtk scripts/release_test.sh` | Reproducible Linux amd64/arm64 executables, matching checksums, and tamper rejection passed. |
| `rtk scripts/deploy-smoke.sh` | Docker Compose and Kubernetes examples validated. |
| `rtk scripts/oci-smoke.sh dans-oci-freeze linux/arm64 v0.0.0-freeze` | Passed; image reports `linux/arm64`, runs as `65532:65532`, and exposes the expected version/help/migration command surface. |
| `rtk scripts/oci-smoke.sh dans-oci-freeze linux/amd64 v0.0.0-freeze` | Passed; image reports `linux/amd64`, runs as `65532:65532`, and exposes the expected version/help/migration command surface. |
...
| Command | Result |
| --- | --- |
| `rtk make generate-check` | OpenAPI and sqlc generation matched checked-in output. |
| `rtk go test ./... -count=1` | 468 tests passed across 16 packages. |
| `rtk go test -race ./... -count=1` | 468 tests passed across 16 packages with the race detector. |
| `rtk go vet ./...` | Clean. |
| `rtk go mod verify` | All modules verified. |
| `rtk go tool sqlc vet -f sqlc.yaml` | Clean. |
| `rtk make benchmark-check` | Portable benchmark correctness cases ran. |
| `rtk make benchmark-postgres-check` | Real PostgreSQL 16.14 authorization benchmark cases ran. |
| `rtk scripts/release_test.sh` | Reproducibility, checksums, tamper rejection, and the 24-MiB executable ceiling passed. |
| `rtk make integration-contract` | CI, unpacked-rootfs, native-Linux RSS, and real-system harness contracts passed. |
| `rtk openspec validate build-dans-v1 --strict --no-interactive` | Change valid in strict mode. |
| `rtk git diff --check` | Clean. |
...
```text
rtk scripts/integration.sh postgres:16.14
integration: PostgreSQL 16.14 passed
rtk scripts/integration.sh postgres:18.4
