...
```sh
env GOWORK=off GOFLAGS=-mod=readonly scripts/release.sh build baseline /tmp/dans-release-baseline
scripts/release.sh verify /tmp/dans-release-baseline
```

Canonical OCI artifacts:

```sh
scripts/oci-smoke.sh dans-baseline linux/amd64 baseline
scripts/oci-smoke.sh dans-baseline linux/arm64 baseline
```
...
```sh
go build -o /tmp/startup-harness ./openspec/changes/build-dans-v1/evidence/performance/startup-harness.go
/tmp/startup-harness 5 50 /tmp/dans-release-baseline/dans_baseline_linux_arm64 version
```

Repeated native-Linux runtime sampling through the real integration topology:

```sh
env DANS_QA_LOG_DIR=/tmp/dans-integration-logs \
...
```sh
go test ./internal/identifier ./internal/dnsname ./internal/httpapi ./internal/httpserver -run '^$' -bench '^Benchmark' -benchtime=3s -count=5 -benchmem
```

PostgreSQL authorization baseline against a fresh supported database:

```sh
env DANS_TEST_DATABASE_URL='postgres://USER:PASSWORD@HOST/DATABASE?sslmode=disable' go test -tags=integration ./internal/database -run '^$' -bench '^BenchmarkAuthorizeRRsetBatch$' -benchtime=3s -count=5 -benchmem
```

Statistical summary:

```sh
go run golang.org/x/perf/cmd/benchstat@v0.0.0-20260615155930-9e4b9ddef5b6 openspec/changes/build-dans-v1/evidence/performance/benchmarks.txt
```
