...
merge_running_records() {
	...
	wait_supervised() {
		while [ ! -e "$running_launch_marker.finished" ]; do
			process_running "$running_pid" || {
				printf '%s\n' 'dev lifecycle: supervisor exited before completion was recorded' >&2
				mark_operation_uncertain
				return 125
			}
		capture_process_tree "$running_pid"
	...
	[ ! -e "$running_launch_marker.escaped" ] || {
		printf '%s\n' 'dev lifecycle: supervisor watchdog detected an escaped process' >&2
		mark_operation_uncertain
		return 125
	}
	...
	case "$running_status" in
		''|*[!0-9]*)
			printf '%s\n' 'dev lifecycle: supervisor status was not verifiable' >&2
			mark_operation_uncertain
			return 125
			;;
		*) ;;
	...
	if [ "$running_status" -ne 0 ] && [ "$running_launch_capture_stderr" -eq 1 ] &&
		...
		printf '%s\n' 'dev lifecycle: Docker transport failure made completion uncertain' >&2
		mark_operation_uncertain
		return 125
	fi
